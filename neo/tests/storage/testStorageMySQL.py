...
from neo.storage.database.mysql import (MySQLDatabaseManager,
    ...
    SERVER_GONE_ERROR, UNKNOWN_STORAGE_ENGINE, bytes_literal)
...
class StorageMySQLdbTests(StorageDBTests):
    ...
    def test_max_allowed_packet(self):
        ...
        self.assertEqual(4, max(len(bytes_literal(bytes(bytearray((x,))))) # PY3
                                for x in range(256)))
        self.assertEqual(4, len(bytes_literal(b'\0')))
