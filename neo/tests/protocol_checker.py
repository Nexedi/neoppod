...
class parseArgument(ast.NodeTransformer):
    ...
    def visit_UnaryOp(self, node):
        ...
        return _ast.Call(_ast.Name('option', ast.Load()),
            [self.visit(node.operand)], [], *() if six.PY3 else (None, None))
...
class Argument(object):
    ...
    def _load(cls, arg):
        ...
        if t is tuple:
            x.type = list(map(cls._load, arg))
        elif t is list:
            x.type = cls._load(*arg),
        elif t is dict:
            (k, v), = six.iteritems(arg)
    ...
    def __init__(self, arg, root=False):
        if arg is None:
            ...
            self.type = list(map(self.__class__, arg))
        elif isinstance(arg, array):
            self.type = self._merge(arg),
        elif isinstance(arg, dict):
            self.type = tuple(map(self._merge, zip(*arg.items())))
        else:
            self.type = (('p64' if len(arg) == 8 else
                'bin')          if isinstance(arg, bytes)   else
                arg._enum._name if isinstance(arg, item)    else
                'str'           if six.PY2 and isinstance(arg, unicode) else
                'int'           if six.PY2 and isinstance(arg, long)    else
                type(arg).__name__)
...
def protocolChecker(dump):
    ...
    if dump:
        with open(path, 'w') as p:
            ...
            for x in sorted(six.iteritems(schema)):
                p.write('%s%r\n' % x)
