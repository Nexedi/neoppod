...
try:
    ...
    BUGGY_LOCK = (implementation.name == 'cpython' and
                  implementation.version < (3, 13)) # (3..3.8 not tested)
    if BUGGY_LOCK:
        # WKRD: Acquiring a released lock sometimes leaves it released,
        #       breaking the above implementation (crash when trying to
        #       release). The issue can be easily reproduced with GCTests.test1
        #       or tools/stress.
