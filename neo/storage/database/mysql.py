...
if useMySQLdb():
    ...
    if six.PY3:
        string_literal = lambda x: bytes_literal(x.encode("ascii"))
    else:
...
SQL_DATADIR = (b"SELECT * FROM information_schema.global_variables"
               b" WHERE variable_name='datadir'")
...
class MySQLDatabaseManager(MVCCDatabaseManager):
    ...
    def storePackOrder(self, tid, approved, partial, oid_list, pack_tid):
        ...
        self.query(b"INSERT INTO pack VALUES (%u,%s,%u,%s,%u)" % (
            ...
            partial,
            b'NULL' if oid_list is None else bytes_literal(b''.join(oid_list)),
            u64(pack_tid)))
