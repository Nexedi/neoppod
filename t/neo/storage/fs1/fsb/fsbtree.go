...
func (t *Tree) extract(q *d, i int) { // (r zodb.Tid) {
	t.ver++
...
func (t *Tree) First() (k zodb.Oid, v zodb.Tid) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *Tree) Get(k zodb.Oid) (v zodb.Tid, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *Tree) insert(q *d, i int, k zodb.Oid, v zodb.Tid) *d {
	t.ver++
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *Tree) Last() (k zodb.Oid, v zodb.Tid) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *Tree) overflow(p *x, q *d, pi, i int, k zodb.Oid, v zodb.Tid) {
	t.ver++
	l, r := p.siblings(pi)
...
func (t *Tree) Set(k zodb.Oid, v zodb.Tid) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
func (t *Tree) Put(k zodb.Oid, upd func(oldV zodb.Tid, exists bool) (newV zodb.Tid, write bool)) (oldV zodb.Tid, written bool) {
	pi := -1
	var p *x
	q := t.r
	var newV zodb.Tid
...
func (t *Tree) split(p *x, q *d, pi, i int, k zodb.Oid, v zodb.Tid) {
	t.ver++
	r := btDPool.Get().(*d)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *Enumerator) Next() (k zodb.Oid, v zodb.Tid, err error) {
...
// == io.EOF is returned.
func (e *Enumerator) Prev() (k zodb.Oid, v zodb.Tid, err error) {
	if err = e.err; err != nil {
		return
	}
